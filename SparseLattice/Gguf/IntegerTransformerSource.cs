...
/// <summary>
/// CPU-only <see cref="IEmbeddingSource"/> running a BERT-family encoder forward pass
/// entirely in integer arithmetic. Weight quantization at load time (float → long at
/// 2^30 scale), then exact Int128 computation through all 12 layers.
/// </summary>
public sealed partial class IntegerTransformerSource : IEmbeddingSource, IDisposable
{
    ...
    private readonly long[] m_tokenTypeEmbedding;
    ...
    private const int s_ropeMaxSeqLen = 512;
    ...
    /// <summary>Full integer forward pass, dequantized and L2-normalized to float[].</summary>
    ...
    /// <summary>Full integer forward pass returning raw long[] at working scale.</summary>
    public long[] Forward(string text)
    {
        ...
        IntegerLayerNorm.ApplyInPlace(x, seqLen, m_nEmbd, m_embdNormW, m_embdNormB, -m_scaleBits);
